...
internal static class LdapAdapter
{
    public static async Task RetrieveClaimsAsync(LdapSettings settings, ClaimsIdentity identity, ILogger logger)
    {
        ...
        if (searchResponse.Entries.Count > 0)
        {
            ...
            {
                // Example distinguished name: CN=TestGroup,DC=KERB,DC=localor something like that
    ...
    private static void GetNestedGroups(LdapConnection connection, ClaimsIdentity principal,
    string distinguishedName, string groupCN,
    ILogger logger, IList<string> retrievedClaims,
